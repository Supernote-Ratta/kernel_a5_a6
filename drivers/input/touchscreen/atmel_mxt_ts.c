...
#define pr_fmt(fmt) "ratta_atmel_ctp:" fmt

#ifdef CONFIG_TOUCHSCREEN_RATTA
extern int ratta_mt_probe(struct device *dev);
extern void ratta_mt_remove(void);
extern int ratta_mt_record(int id, int x, int y, bool state);
#endif
...
static int mxt_read_t100_config(struct mxt_data *data)
{
	...
	pr_debug("%s : xy_switch=%d\n",__func__,data->xy_switch);
	/* allocate aux bytes */
	error =  __mxt_read_reg(client,
	...
	//ratta fix cap  Resolution  != display Resolution;
	if(data->xy_switch)
		data->max_y = 1400;
	else
		data->max_x = 1400;
...
{
	struct mxt_data *data = dev_get_drvdata(dev);
	return scnprintf(buf, PAGE_SIZE, "%u.%u.%02X.%06x\n",
			 data->info->version >> 4, data->info->version & 0xf,
			 data->info->build,data->config_crc);
}
...
static int mxt_sysfs_init(struct mxt_data *data)
{
	...
	if(client->addr == 0x4a)
		ver = proc_symlink("cap_fw_ver", NULL, "/sys/bus/i2c/devices/2-004a/version");
	else if(client->addr == 0x4b)
		ver = proc_symlink("cap_fw_ver", NULL, "/sys/bus/i2c/devices/2-004b/version");

	if ( !ver )
		dev_err(&client->dev," creat cap_fw_ver error \n");
...
static const struct mxt_platform_data *mxt_parse_dt(struct i2c_client *client)
{
	...
	pdata->gpio_power = of_get_named_gpio_flags(np, "atmel,power-gpio",
						    0, NULL);
...
static int mxt_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	error = devm_gpio_request_one(&client->dev, pdata->gpio_power,
			GPIOF_OUT_INIT_HIGH, "atmel_power_gpio");
	if (error < 0) {
		dev_err(&client->dev,
