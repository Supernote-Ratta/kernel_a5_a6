...
reset:
	...
	if (rc < 0) {
		dev_err(cd->dev, "%s: Error on check_and_deassert_int descriptor r=%d\n",
			__func__, rc);
	}
 ...
 * Show Firmware version via sysfs
 */
static ssize_t cyttsp5_version_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct cyttsp5_core_data *cd = dev_get_drvdata(dev);
	struct cyttsp5_cydata *cydata = &cd->sysinfo.cydata;
	return sprintf(buf, "%02X.%02X.%04X\n" ,
		       cydata->fw_ver_major,cydata->fw_ver_minor, cydata->fw_ver_conf);
}
...
static struct device_attribute attributes[] = {
	...
	__ATTR(version, S_IRUGO, cyttsp5_version_show, NULL),
...
static int add_sysfs_interfaces(struct device *dev)
{
	...
	ver = proc_symlink("cap_fw_ver", NULL, "/sys/bus/i2c/devices/2-0024/version");
	if ( !ver )
		dev_err(dev," creat cap_fw_ver error \n");
