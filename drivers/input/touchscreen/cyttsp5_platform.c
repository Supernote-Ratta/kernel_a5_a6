...
{
	int pwr_en_gpio = pdata->pwr_en_gpio;
	int rc = 0;

	rc = gpio_request(pwr_en_gpio, NULL);
	if (rc < 0) {
		gpio_free(pwr_en_gpio);
		rc = gpio_request(pwr_en_gpio, NULL);
	}
	...
	if (rc < 0) {
		...
		rc = gpio_direction_output(pwr_en_gpio, 1);
		if (rc < 0) {
			pr_err("%s: Fail set output gpio=%d\n",
	...
	dev_info(dev, "%s: INIT CYTTSP pwr_en_gpio gpio=%d  r=%d\n",
		__func__, pwr_en_gpio, rc);
	return 0;
}
		...
		struct device *dev, atomic_t *ignore_irq)
{
	...
	if (rc < 0) {
		...
		rc = gpio_direction_output(pwr_en_gpio, 0);
		if (rc < 0) {
			pr_err("%s: Fail set output gpio=%d\n",
				__func__, pwr_en_gpio);
			gpio_free(pwr_en_gpio);
