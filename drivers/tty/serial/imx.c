...
//added by wym 2018-10-09..................................................
#define UART4_IOMUX_PAD_BASE   				(0X20E0244)
#define UART4_IOMUX_PAD_REG_OFFSET         	 	(16)

#define  UART4_IOMUX_MUXCTL_BASE    			(0X20E054C)
#define UART4_IOMUX_MUXCTL_REG_OFFSET       		(16)


#define REG_CNT    (4)
int  uart4_iomux_pad_reg[REG_CNT];
int  uart4_iomux_muxctl_reg[REG_CNT];

#define SD3_IOMUX_PAD_BASE   					(0X20E0280)
#define SD3_IOMUX_PAD_REG_OFFSET         	 		(24)

#define  SD3_IOMUX_MUXCTL_BASE    				(0X20E0588)
#define SD3_IOMUX_MUXCTL_REG_OFFSET       		(24)
#define SD3_REG_CNT    (6)

int  sd3_iomux_pad_reg[SD3_REG_CNT];
int  sd3_iomux_muxctl_reg[SD3_REG_CNT];

#define  GPIO5BASE  0X20AC000
#define GPIO5GDIR   0x04


#define GPIO5IMR   0x14

#define UART4_GPIO5GDIR    (0XFF30E97F)
int gpio5_gdir;
int gpio5intmask;

void  uart4_iomux_suspend(void)
{
	static void *p_gpj0_base;
	int i;

	p_gpj0_base = ioremap(UART4_IOMUX_PAD_BASE, UART4_IOMUX_PAD_REG_OFFSET);
	if (!p_gpj0_base)
		return;

	for(i=0;i<REG_CNT;i++)
	{
		uart4_iomux_pad_reg[i]=readl(p_gpj0_base+i*4);
		writel(0x05,p_gpj0_base+i*4);    //set to gpio mode
	}
	iounmap(p_gpj0_base);
	p_gpj0_base = NULL;

	p_gpj0_base = ioremap(UART4_IOMUX_MUXCTL_BASE, UART4_IOMUX_MUXCTL_REG_OFFSET);
	if (!p_gpj0_base)
		return;
	for(i=0;i<REG_CNT;i++)
	{
		uart4_iomux_muxctl_reg[i]=readl(p_gpj0_base+i*4);
		writel(0x10000,p_gpj0_base+i*4);    //set pad  to float
	}
	iounmap(p_gpj0_base);
	p_gpj0_base = NULL;

	p_gpj0_base = ioremap(SD3_IOMUX_PAD_BASE, SD3_IOMUX_PAD_REG_OFFSET);
	if (!p_gpj0_base)
		return;
	for(i=0;i<SD3_REG_CNT;i++)
	{
		sd3_iomux_pad_reg[i]=readl(p_gpj0_base+i*4);
		writel(0x05,p_gpj0_base+i*4);    //set to gpio mode
	}
	iounmap(p_gpj0_base);
	p_gpj0_base = NULL;

	p_gpj0_base = ioremap(SD3_IOMUX_MUXCTL_BASE, SD3_IOMUX_MUXCTL_REG_OFFSET);
	if (!p_gpj0_base)
		return;
	for(i=0;i<SD3_REG_CNT;i++)
	{
		sd3_iomux_muxctl_reg[i]=readl(p_gpj0_base+i*4);
		writel(0x10000,p_gpj0_base+i*4);    //set pad  to float
	}
	iounmap(p_gpj0_base);
	p_gpj0_base = NULL;

	p_gpj0_base = ioremap(GPIO5BASE, 20);
	if (!p_gpj0_base)
		return;
	gpio5_gdir=readl(p_gpj0_base+GPIO5GDIR);
	writel(gpio5_gdir&UART4_GPIO5GDIR,p_gpj0_base);    //set GPIO INPUT

	gpio5intmask=readl(p_gpj0_base+GPIO5IMR);
...
void  uart4_iomux_resum(void)
{
	...
	if (!p_gpj0_base)
		return;
	for(i=0;i<REG_CNT;i++) {
	//	epdc_iomux_pad_reg[i]=readl(p_gpj0_base+i*4);
		writel(uart4_iomux_pad_reg[i],p_gpj0_base+i*4);    //set to gpio mode
	}
	iounmap(p_gpj0_base);
	p_gpj0_base = NULL;
	...
	if (!p_gpj0_base)
		return;
	for(i=0;i<REG_CNT;i++) {
	//	epdc_iomux_muxctl_reg[i]=readl(p_gpj0_base+i*4);
		writel(uart4_iomux_muxctl_reg[i],p_gpj0_base+i*4);    //set pad  to float
	}
	iounmap(p_gpj0_base);
	p_gpj0_base = NULL;
	...
	for(i=0;i<SD3_REG_CNT;i++) {
	//	sd3_iomux_pad_reg[i]=readl(p_gpj0_base+i*4);
		writel(sd3_iomux_pad_reg[i],p_gpj0_base+i*4);    //set to gpio mode
	}
	iounmap(p_gpj0_base);
	p_gpj0_base = NULL;
	...
	for(i=0;i<SD3_REG_CNT;i++) {
	//	sd3_iomux_muxctl_reg[i]=readl(p_gpj0_base+i*4);
		writel(sd3_iomux_muxctl_reg[i],p_gpj0_base+i*4);    //set pad  to float, 
	}
	iounmap(p_gpj0_base);
	p_gpj0_base = NULL;
	...
	if (!p_gpj0_base)
		return;
//	gpio1_gdir=readl(p_gpj0_base);
	writel(gpio5_gdir,p_gpj0_base+GPIO5GDIR);    //set GPIO INPUT

	//p_gpj0_base = ioremap(GPIO5IMR, 4);

	writel(gpio5intmask,p_gpj0_base+GPIO5IMR); 
	iounmap(p_gpj0_base);
	p_gpj0_base = NULL;
}
//end added  ...................................................................................................
static int serial_imx_suspend(struct platform_device *dev, pm_message_t state)
{
	...
	unsigned int val;
	printk("wym serial_imx_suspend,line:%d\n",sport->port.line);
...
static int serial_imx_resume(struct platform_device *dev)
{
	struct imx_port *sport = platform_get_drvdata(dev);
	unsigned int val;
	printk("wym serial_imx_resume line:%d\n",sport->port.line);
	if(sport->port.line==3)
		uart4_iomux_resum();
