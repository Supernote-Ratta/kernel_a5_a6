...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 0, 0) || defined(WL_SUPPORT_BACKPORTED_KPATCHES)) && \
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 19, 0) || (defined(WL_COMPAT_WIRELESS) && !defined(WL_COMPAT_2_6_X)))
...
#if (defined(CONFIG_ARCH_MSM) && defined(TDLS_MGMT_VERSION2)) || (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 19, 0)) || defined(WL_COMPAT_WIRELESS)
	...
	struct vif_params *params)
{
	...
	if (cfg->p2p_supported && (wlif_type != -1)) {
		...
		if (timeout > 0 && !wl_get_p2p_status(cfg, IF_ADDING) && cfg->if_event_info.valid) {
			...
			new_ndev->name_assign_type = name_assign_type;
#endif
...
#if (LINUX_VERSION_CODE < KERNEL_VERSION(3, 14, 0) && !defined(WL_COMPAT_WIRELESS))
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 19, 0) || (defined(WL_COMPAT_WIRELESS) && !defined(WL_COMPAT_2_6_X)))
static void wl_convert_sta_flags(s32 fw_sta_flags, struct station_info *si)
{
	...
	if (fw_sta_flags & WLC_STA_WME)
		sfu->set |= BIT(NL80211_STA_FLAG_WME);
	if (fw_sta_flags & WLC_STA_AUTHE)
		sfu->set |= BIT(NL80211_STA_FLAG_AUTHENTICATED);
	if (fw_sta_flags & WLC_STA_ASSOC)
		sfu->set |= BIT(NL80211_STA_FLAG_ASSOCIATED);
	if (fw_sta_flags & WLC_STA_AUTHO)
...
#endif
{
	...
	if (wl_get_mode_by_netdev(cfg, dev) == WL_MODE_AP) {
...
#if (LINUX_VERSION_CODE < KERNEL_VERSION(3, 0, 0) || (defined(WL_COMPAT_WIRELESS) && defined(WL_COMPAT_2_6_X)))
		...
		wl_convert_sta_flags(sta_flags, sinfo);
#endif
...
#endif
			sinfo->rx_packets = dtoh32(pktcnt.rx_good_pkt);
			sinfo->rx_dropped_misc = dtoh32(pktcnt.rx_bad_pkt);
			sinfo->tx_packets = dtoh32(pktcnt.tx_good_pkt);
			sinfo->tx_failed  = dtoh32(pktcnt.tx_bad_pkt);
		}
get_station_err:
		if (err)
			err_cnt++;
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 19, 0) || (defined(WL_COMPAT_WIRELESS) && !defined(WL_COMPAT_2_6_X)))
			/* cfg80211_disconnected(dev, 0, NULL, 0, true, GFP_KERNEL); modified by ratta */
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 19, 0) || (defined(WL_COMPAT_WIRELESS) && !defined(WL_COMPAT_2_6_X)))
	scb_val.val = htod32(params->reason_code);
#else
	...
	struct cfg80211_ap_settings *info)
{
	...
	if(info->inactivity_timeout) {
		printk("%s: inactivity timeout = %d \n", __func__, info->inactivity_timeout);
        	err = wldev_ioctl(dev, WLC_SET_SCB_TIMEOUT, &info->inactivity_timeout, sizeof(u32), true);
        	if (err < 0) {
            		WL_ERR(("AP set inactivity timeout error %d\n", err));
            		goto fail;
        	}
	}
...
#ifdef CONFIG_PM
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 11, 0) || defined(WL_COMPAT_WIRELESS))
...
static s32 wl_setup_wiphy(struct wireless_dev *wdev, struct device *sdiofunc_dev, void *context)
{
	...
	wdev->wiphy->features |= NL80211_FEATURE_INACTIVITY_TIMER;
#endif
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 9, 0)) || defined(WL_SUPPORT_BACKPORTED_KPATCHES)
...
#if (LINUX_VERSION_CODE < KERNEL_VERSION(3, 11, 0) && !defined(WL_COMPAT_WIRELESS)) || \
		(defined(WL_COMPAT_WIRELESS) && defined(WL_COMPAT_2_6_X))
	...
	const wl_event_msg_t *e, void *data)
{
	...
	u32 status =  ntoh32(e->status);
	bool active;
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 15, 0) || (defined(WL_COMPAT_WIRELESS) && !defined(WL_COMPAT_2_6_X)))
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 19, 0) || (defined(WL_COMPAT_WIRELESS) && !defined(WL_COMPAT_2_6_X)))
					/* cfg80211_disconnected(ndev, reason, NULL, 0, true, GFP_KERNEL); */ /* modified by ratta */
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 19, 0) || (defined(WL_COMPAT_WIRELESS) && !defined(WL_COMPAT_2_6_X)))
	/* cfg80211_disconnected(ndev , 0, NULL, 0, true, GFP_KERNEL); modified by ratta	*/
...
#undef LOCAL_BUF_LEN
	...
	for (i = 0; i < __wl_band_2ghz.n_channels; i++)
		__wl_band_2ghz.channels[i].flags = IEEE80211_CHAN_DISABLED;

	for (i = 0; i < __wl_band_5ghz_a.n_channels; i++)
		__wl_band_5ghz_a.channels[i].flags = IEEE80211_CHAN_DISABLED;
	...
	for (i = 0; i < dtoh32(list->count); i++) {
		...
		if (!CHSPEC_IS40(c) && !CHSPEC_IS20(c) && !CHSPEC_IS80(c)) {
		...
		if (CHSPEC_IS2G(c) && (channel >= CH_MIN_2G_CHANNEL) &&
			...
			n_cnt = &n_2g;
			band = IEEE80211_BAND_2GHZ;
			ht40_allowed = (bw_cap[band] == WLC_BW_40MHZ_BIT)? true : false;
		} else if (CHSPEC_IS5G(c) && channel >= CH_MIN_5G_CHANNEL) {
			band_chan_arr = __wl_5ghz_a_channels;
			array_size = ARRAYSIZE(__wl_5ghz_a_channels);
			n_cnt = &n_5g;
			band = IEEE80211_BAND_5GHZ;
			ht40_allowed = (bw_cap[band] == WLC_BW_20MHZ_BIT)? false : true;
		} else {
		...
		if (!ht40_allowed && CHSPEC_IS40(c))
			continue;
		if (!(bw_cap[band] & WLC_BW_40MHZ_BIT) && CHSPEC_IS40(c))
			continue;
		if (!(bw_cap[band] & WLC_BW_80MHZ_BIT) && CHSPEC_IS80(c))
			continue;
		...
		if (index <  array_size) {
			...
			if (CHSPEC_IS80(c)) {
				...
				band_chan_arr[index].flags = IEEE80211_CHAN_NO_HT40 | IEEE80211_CHAN_NO_80MHZ;
...
static void wl_get_bwcap(struct net_device *dev, u32 bw_cap[2])
{
	u32 band, mimo_bwcap;
	int err;
...
static u16 wl_get_mcs_map(u32 nchain, enum ieee80211_vht_mcs_support supp)
{
	u16 mcs_map;
	int i;
...
static void wl_update_vht_cap(struct ieee80211_supported_band *band,
				 u32 bw_cap[2], u32 nchain)
{
	u16 mcs_map;
	...
	/* all support 256-QAM */
	mcs_map = wl_get_mcs_map(nchain, IEEE80211_VHT_MCS_SUPPORT_0_9);
...
s32 wl_update_wiphybands(struct bcm_cfg80211 *cfg, bool notify)
{
	...
	(void)wldev_iovar_getint(dev, "vhtmode", &vhtmode);
	err = wldev_iovar_getint(dev, "nmode", &nmode);
	if (unlikely(err)) {
		WL_ERR(("error reading nmode (%d)\n", err));
	} else {
		wl_get_bwcap(dev, bw_cap);
	}
	...
	for (i = 1; i <= nband && i < ARRAYSIZE(bandlist); i++) {
		...
		if ((index >= 0) && nmode) {
			wl_update_ht_cap(bands[index], bw_cap, nchain);
		}
		if ((index >= 0) && vhtmode) {
			wl_update_vht_cap(bands[index], bw_cap, nchain);
		}
	}
...
int wl_cfg80211_hang(struct net_device *dev, u16 reason)
{
	...
	wl_add_remove_pm_enable_work(cfg, FALSE, WL_HANDLER_DEL);
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 19, 0) || (defined(WL_COMPAT_WIRELESS) && !defined(WL_COMPAT_2_6_X)))
	/* cfg80211_disconnected(dev, reason, NULL, 0, true, GFP_KERNEL); modified by ratta */
...
#if defined(CONFIG_ARCH_MSM) && defined(TDLS_MGMT_VERSION2) || defined(WL_COMPAT_WIRELESS)
