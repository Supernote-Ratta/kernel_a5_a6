...
#endif	/* !CONFIG_OF */
extern int fpga_pwren;
extern int fpga_pwrenold;
...
#if 1
/*
	tps6518x->gpio_pmic_powerup = of_get_named_gpio(np,
					"gpio_pmic_powerup", 0);
	if (!gpio_is_valid(tps6518x->gpio_pmic_powerup)) {
		dev_err(tps6518x->dev, "no epdc pmic pwr_en pin available\n");
		goto err1;
	}
	printk("\n*************tps6518x->gpio_pmic_powerup=0x%X \n",tps6518x->gpio_pmic_powerup );
	ret = devm_gpio_request_one(tps6518x->dev, tps6518x->gpio_pmic_powerup,
				GPIOF_OUT_INIT_HIGH, "gpio_pmic_pwr");
	if (ret < 0)
		goto err1;
*/

	tps6518x->gpio_pmic_pwr_en = of_get_named_gpio(np,
					"gpio_pmic_pwr_en", 0);
	...
	ret = devm_gpio_request_one(tps6518x->dev, tps6518x->gpio_pmic_pwr_en,
				GPIOF_OUT_INIT_HIGH, "gpio_pmic_pwr");
	if (ret < 0)
		goto err1;
...
//fpga_pwren
	...
	fpga_pwren = of_get_named_gpio(np,
						"fpga_pwren", 0);
		if (!gpio_is_valid(fpga_pwren)) {
			dev_err(tps6518x->dev, "no epdc pmic pwr_en pin available\n");
			goto err1;
		}
		printk("\n*************fpga_pwren=0x%X \n",fpga_pwren);
		ret = devm_gpio_request_one(tps6518x->dev, fpga_pwren,
					GPIOF_OUT_INIT_HIGH, "fpga_pwren");
		if (ret < 0)
			goto err1;

	fpga_pwrenold = of_get_named_gpio(np,
						"fpga_pwrenold", 0);
		if (!gpio_is_valid(fpga_pwrenold)) {
			dev_err(tps6518x->dev, "no epdc pmic pwr_en pin available\n");
			goto err1;
		}
		printk("\n*************fpga_pwrenold=0x%X \n",fpga_pwrenold);
		ret = devm_gpio_request_one(tps6518x->dev, fpga_pwrenold,
					GPIOF_OUT_INIT_HIGH, "fpga_pwren");
		if (ret < 0)
			goto err1;
	...
	int usb_wakeup_ctrl;
	usb_wakeup_ctrl = of_get_named_gpio(np,
						"usb_wakeup_ctrl", 0);
		if (!gpio_is_valid(usb_wakeup_ctrl)) {
			dev_err(tps6518x->dev, "no epdc pmic pwr_en pin available\n");
			goto err1;
		}

		ret = devm_gpio_request_one(tps6518x->dev, usb_wakeup_ctrl,
					GPIOF_OUT_INIT_HIGH, "usb_wakeup_ctrl");
		if (ret < 0)
			goto err1;


	tps6518x->gpio_pmic_vddio_en = of_get_named_gpio(np,
					"gpio_pmic_vddio_en", 0);
	if (!gpio_is_valid(tps6518x->gpio_pmic_vddio_en)) {
		dev_err(tps6518x->dev, "no epdc pmic vddio_en pin available\n");
		goto err1;
	}
	printk("\n*************tps6518x->gpio_pmic_vddio_en=0x%X \n",tps6518x->gpio_pmic_vddio_en );
	ret = devm_gpio_request_one(tps6518x->dev, tps6518x->gpio_pmic_vddio_en,
				GPIOF_OUT_INIT_HIGH, "gpio_pmic_vddio");
	if (ret < 0)
		goto err1;
#endif

	tps6518x->gpio_pmic_wakeup = of_get_named_gpio(np,
					"gpio_pmic_wakeup", 0);
	if (!gpio_is_valid(tps6518x->gpio_pmic_wakeup)) {
		dev_err(tps6518x->dev, "no epdc pmic wakeup pin available\n");
		goto err1;
	}
	printk("\n*************tps6518x-core tps6518x->gpio_pmic_wakeup=0x%X \n",tps6518x->gpio_pmic_wakeup );
	ret = devm_gpio_request_one(tps6518x->dev, tps6518x->gpio_pmic_wakeup,
...
#else
#define TPS6518X_WAKEUP_GPIO ((2-1)*32+9)
		//gpio_request(TPS6518X_WAKEUP_GPIO, "TPS6518X_WAKEUP");
		gpio_direction_output(TPS6518X_WAKEUP_GPIO, 0);
		gpio_set_value(TPS6518X_WAKEUP_GPIO, 1);
		msleep(5);
		printk("\n!!!!!!!!!!!!!!!!!\n!!!!!!!!!!!!!!!!\n");
#endif	
	...
	if(i==10)	{
		dev_err(tps6518x->dev, "can't detect\n");
		goto err1;
	}
