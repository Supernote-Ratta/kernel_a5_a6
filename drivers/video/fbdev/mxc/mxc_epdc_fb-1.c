...
//#define EPDC_MANAGE_LUT
//#define GPIO1_BCM_WL_REG_ON111   ((4-1)*32+i)
#define EPDC_PMIC_PWR_EN    ((2-1)*32+8)//GPIO2  IO08
#define EINK_VDDIO_EN    ((2-1)*32+7)//GPIO2  IO07
#define EPDC_GDSP		((2-1)*32+2)
#define EPDC_SDCE0	((1-1)*32+27)
#ifdef EPDC_MANAGE_LUT
static u64 epdc_used_luts = 0x1;	/* do not use LUT0 */
#endif
int fpga_pwren=0;  //wym 2018-01-22
...
#ifdef EPDC_MANAGE_LUT
static inline void epdc_set_used_lut(u32 used_bit);
static inline void epdc_clear_used_lut(u32 used_bit);
...
static inline void epdc_set_used_lut(u32 used_bit)
{
	epdc_used_luts |= (u64)1 << used_bit;
	//printk("%s, set lut: %d\n", __FUNCTION__, used_bit);
}
...
{
#if 1
	mutex_lock(&fb_data->queue_mutex);	 //修改这里，添加一把锁
#endif
	mutex_lock(&fb_data->power_mutex);
	...
	mutex_unlock(&fb_data->power_mutex);
#if 1
     mutex_unlock(&fb_data->queue_mutex); //修改这里，添加一把锁；避免出现别的地方正在执行什么操作的时候，另一个线程调关机。利用线程锁同步起来。	
#endif
}
				   ...
				   struct mxc_epdc_fb_data *fb_data)
{
	...
	mdelay(15);
#else
	printk("wym epdc_process_update \n");
	printk("wym epdc_process_update 1 \n");
	printk("wym epdc_process_update 2\n");
	printk("wym epdc_process_update 4\n");
#endif
	...
	mdelay(10);
#else
	 printk("wym epdc_process_update 5\n");
	 printk("wym epdc_process_update 6\n");
	 printk("wym epdc_process_update 7\n");
#endif
	...
	mdelay(5);
#else	
	printk("wym epdc_process_update end\n");   //5ms
#endif	
return 0;
}
...
#if 1
	if(upd_data_list->update_desc->upd_data.waveform_mode==0x02)
	         {
	             int  opdelay;
	             do_gettimeofday(&tv1);
	             opdelay=((tv1.tv_sec*1000+( tv1.tv_usec/1000))-(tv2.tv_sec*1000+( tv2.tv_usec/1000)));
	             if(opdelay<1000)
	             {
	                 mutex_unlock(&fb_data->queue_mutex);

	                 printk("wym epdc_submit_work_func 9,MODE:%x\n",upd_data_list->update_desc->upd_data.waveform_mode);
	                 if(opdelay<300)
	                     mdelay(800);
	                 else if(opdelay<500)
	                     mdelay(600);
...
int mxc_epdc_fb_probe(struct platform_device *pdev)
{
	...
	u32 val;
	int irq;
	fpga_pwren=1;  //wym 2018-01-22, enable fpga power on
...
#ifdef EPDC_MANAGE_LUT
	epdc_used_luts = 0x1;	/* do not use LUT0 */
#endif
...
//	gpio_set_value(EPDC_PMIC_PWR_EN,0);  //CLOSE THE POWER
//	gpio_set_value(EINK_VDDIO_EN,0);  //
	fpga_pwren=0; //wym 2018-01-22  , make the fpga poweroff 
	return ret;
}
...
//	printk("wym mxc_epdc_fb_resume \n");
//	gpio_set_value(EPDC_PMIC_PWR_EN,1);  //OPEN THE POWER
//	gpio_set_value(EINK_VDDIO_EN,1);  //
...
{
//	printk("wym mxc_epdc_fb_exit \n");
	fpga_pwren=0;  //wym 2018-01-22, enable fpga power off
