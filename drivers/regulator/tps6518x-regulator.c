...
//static int tps6518x_vcom = { -1500000 };
//static int tps6518x_vcom = { -1560000 };
...
{
	struct tps6518x *tps6518x = rdev_get_drvdata(reg);
//	printk("wym tps6518x_v3p3_enable \n");
	gpio_set_value(tps6518x->gpio_pmic_powerup, 1);
	
	gpio_set_value(tps6518x->gpio_pmic_vddio_en, 1);
...
{
	struct tps6518x *tps6518x = rdev_get_drvdata(reg);
//	printk("wym tps6518x_v3p3_disable \n");
//	gpio_set_value(tps6518x->gpio_pmic_powerup, 0);

	tps6518x_reg_write(REG_TPS65180_ENABLE, 0x40);	   //added by wym 20190518

//	gpio_set_value(tps6518x->gpio_pmic_vddio_en, 0);
...
//	printk("wym tps6518x_vcom_enable gpio_pmic_vcom_ctrl :%d,%d\n",tps6518x->gpio_pmic_vcom_ctrl,vcomEnable);
	tps6518x_reg_read(REG_TPS65180_ENABLE, &temp0);
	#endif
//	temp0=temp0|0x10;
	tps6518x_reg_write(REG_TPS65180_ENABLE, 0xbf);
	gpio_set_value(tps6518x->gpio_pmic_vcom_ctrl,1);
//	printk("wym tps6518x_vcom_enable %x\n",temp0);
	return 0;
}
...
static int tps6518x_is_power_good(struct tps6518x *tps6518x)
{
	...
	#else
	int temp;
	 temp=gpio_get_value(tps6518x->gpio_pmic_pwrgood) ^
		tps6518x->pwrgood_polarity;
	 if (temp == 1  )	 	
		 printk("\n RRRRRRRRRRR power_good = 0x%X\n",temp);
	 else
	 	 printk("\n EEEEEEEEEEEE power_good = 0x%X\n",temp);
	 return temp;
	#endif	
#else
	int pg_temp;
	tps6518x_reg_read(REG_TPS6518x_PG, &pg_temp);
	if(pg_temp==0xFA)
	{
		printk("\n oooooooooooooooootps6518x_is_power_good = 0x%X\n",pg_temp);
		return 1;
	}
	else
	{
		printk("\nXXXXXXXXXXXXXXtps6518x_is_power_good = 0x%X\n",pg_temp);
		return 0;
	}
...
//		gpio_set_value(tps6518x->gpio_pmic_powerup, 1);
	...
	#if 0    //added by wym 20190518
		cur_reg_val = tps65180_current_Enable_Register & 0x3f;	  
		fld_mask = BITFMASK(VDDH_EN) | BITFMASK(VPOS_EN) |
			BITFMASK(VEE_EN) | BITFMASK(VNEG_EN) | BITFMASK(V3P3_SW_EN);
		fld_val = BITFVAL(VDDH_EN, true) | BITFVAL(VPOS_EN, true) |
			BITFVAL(VEE_EN, true) | BITFVAL(VNEG_EN, true) /*| BITFVAL(VCOM_EN, true) */| BITFVAL(V3P3_SW_EN, true);
...
#if 0
		int pg_temp;
		tps6518x_reg_write(REG_TPS65180_ENABLE, 0x60);
		msleep(1);
		tps6518x_reg_write(REG_TPS65180_ENABLE, 0xa2);
		msleep(15);
		tps6518x_reg_read(REG_TPS6518x_PG, &pg_temp);
		printk("wym tps6518x_display_enable pg_temp: %x  \n",pg_temp);
		tps6518x_reg_write(REG_TPS65180_ENABLE, 0xa3);
		msleep(15);
		tps6518x_reg_read(REG_TPS6518x_PG, &pg_temp);
		printk("wym tps6518x_display_enable pg_temp: %x  \n",pg_temp);
		tps6518x_reg_write(REG_TPS65180_ENABLE, 0xa7);
		msleep(30);
		tps6518x_reg_read(REG_TPS6518x_PG, &pg_temp);
		printk("wym tps6518x_display_enable pg_temp: %x  \n",pg_temp);
		tps6518x_reg_write(REG_TPS65180_ENABLE, 0xaf);
	...
	//	printk("REG_TPS65180_ENABLE 1=0x%X\n",new_reg_val);
	//	msleep(10);
	//	tps6518x_reg_write(REG_TPS65180_ENABLE, new_reg_val);
		msleep(10);
#if 0   //added by wym 20190518
	...
	//	cur_reg_val = tps65180_current_Enable_Register & 0x3f;
		cur_reg_val = tps65180_current_Enable_Register & 0x2f;

		fld_mask = BITFMASK(ACTIVE);   //0b 1000 0000
		fld_val = BITFVAL(ACTIVE, true);  //0b 1000 0000
		new_reg_val = tps65180_current_Enable_Register = to_reg_val(cur_reg_val, fld_mask, fld_val);
		new_reg_val = tps65180_current_Enable_Register & 0xef;
	//	tps6518x_reg_write(REG_TPS65180_ENABLE, new_reg_val);
	#endif
		tps6518x_reg_write(REG_TPS65180_ENABLE, 0xaf);
	//	printk("REG_TPS65180_ENABLE 2=0x%X\n",new_reg_val);
		gpio_set_value(tps6518x->gpio_pmic_powerup, 1);
#endif	
	}
	msleep(60);
	...
	#if 1
	if(tps6518x_is_power_good(tps6518x)==0)
	{
		recheck=20;
		while(recheck--)
			{
			msleep(5);
			tps6518x_reg_read(REG_TPS65180_INT2,&int2);
			tps6518x_reg_read(REG_TPS65180_INT1,&int1);
			printk("wym tps6518x_display_enable   error %d .........int1:%x,int2:%x.........................................\n",flagcnt,int1,int2);
	//	if(tps6518x_is_power_good(tps6518x)==0)
	/*		if(int2!=0||int1!=0)
			{
	...
	//	gpio_set_value(tps6518x->gpio_pmic_wakeup,1);
			...
			if(tps6518x_is_power_good(tps6518x)==1)
				break;
		}
		}
	else
...
static int tps6518x_display_disable(struct regulator_dev *reg)
{
	...
	else
	{
		...
		new_reg_val = tps65180_current_Enable_Register = to_reg_val(cur_reg_val, fld_mask, fld_val);
		#endif
		tps6518x_reg_write(REG_TPS65180_ENABLE, 0x60);     //added by wym 20190518
		tps6518x_reg_read(REG_TPS65180_INT1,&int1);
		tps6518x_reg_read(REG_TPS65180_INT2,&int2);
	//	printk("wym tps6518x_display_disable int1:%x,int2:%x,new_reg_val:%x \n",int1,int2,new_reg_val);
	}
//	gpio_set_value(tps6518x->gpio_pmic_wakeup,0);
...
//	   printk("wym tps6518x_setup_timings  temp0:%x ,temp1:%x,temp2:%x,temp3:%x\n",temp0,temp1,temp2,temp3);
//	   printk("wym tps6518x_setup_timings  upseq0:%x ,upseq1:%x,dwnseq0:%x,dwnseq1:%x\n",tps6518x->upseq0,tps6518x->upseq1,tps6518x->dwnseq0,tps6518x->dwnseq1);
	   tps6518x_reg_read(REG_TPS65185_UPSEQ0, &temp0);
	tps6518x_reg_read(REG_TPS65185_UPSEQ1, &temp1);
	tps6518x_reg_read(REG_TPS65185_DWNSEQ0, &temp2);
	tps6518x_reg_read(REG_TPS65185_DWNSEQ1, &temp3);
...
#if  1  //add by yan2anfei

	CHECK_PROPERTY_ERROR_KFREE(upseq0);
	CHECK_PROPERTY_ERROR_KFREE(upseq1);
	CHECK_PROPERTY_ERROR_KFREE(dwnseq0);
	CHECK_PROPERTY_ERROR_KFREE(dwnseq1);
	dev_dbg(&pdev->dev, "upseq0 =0x%X \n upseq1 =0x%X \n dwnseq0 =0x%X \n dwnseq1 =0x%X \n", \
		                       tps6518x->upseq0,tps6518x->upseq1,tps6518x->dwnseq0,tps6518x->dwnseq1);	

//	printk("printk: upseq0 =0x%X \n upseq1 =0x%X \n dwnseq0 =0x%X \n dwnseq1 =0x%X \n", \
		                       tps6518x->upseq0,tps6518x->upseq1,tps6518x->dwnseq0,tps6518x->dwnseq1);
#endif

	tps6518x->max_wait = (6 + 6 + 6 + 6);
#if  0
	tps6518x->gpio_pmic_pwr_en = of_get_named_gpio(pmic_np,
					"gpio_pmic_pwr_en", 0);
	if (!gpio_is_valid(tps6518x->gpio_pmic_pwr_en)) {
		dev_err(&pdev->dev, "no epdc pmic pwr_en pin available\n");
		goto err;
	}
	printk("\n*************tps6518x->gpio_pmic_pwr_en=0x%X \n",tps6518x->gpio_pmic_pwr_en );
	ret = devm_gpio_request_one(&pdev->dev, tps6518x->gpio_pmic_pwr_en,
				GPIOF_OUT_INIT_HIGH, "epdc-pmic-wake");
	if (ret < 0)
		goto err;

	tps6518x->gpio_pmic_vddio_en = of_get_named_gpio(pmic_np,
					"gpio_pmic_vddio_en", 0);
	if (!gpio_is_valid(tps6518x->gpio_pmic_vddio_en)) {
		dev_err(&pdev->dev, "no epdc pmic vddio_en pin available\n");
		goto err;
	}
	printk("\n*************tps6518x->gpio_pmic_vddio_en=0x%X \n",tps6518x->gpio_pmic_vddio_en );
	ret = devm_gpio_request_one(&pdev->dev, tps6518x->gpio_pmic_vddio_en,
				GPIOF_OUT_INIT_HIGH, "epdc-pmic-wake");
	if (ret < 0)
		goto err;
	...
	printk("\n*************tps6518x->gpio_pmic_wakeup=0x%X \n",tps6518x->gpio_pmic_wakeup );
	ret = devm_gpio_request_one(&pdev->dev, tps6518x->gpio_pmic_wakeup,
				GPIOF_OUT_INIT_HIGH, "epdc-pmic-wake");
	if (ret < 0)
		goto err;
#else
//	printk("\n*************tps6518x-regulator tps6518x->gpio_pmic_wakeup=0x%X \n",tps6518x->gpio_pmic_wakeup );
#endif
//	CD4052_A0

	CD4052_A0 = of_get_named_gpio(pmic_np,
						"CD4052_A0", 0);
		if (!gpio_is_valid(CD4052_A0)) {
			dev_err(&pdev->dev, "no epdc pmic vcom_ctrl pin available\n");
			goto err;
		}
	//	printk("\n*************tps6518x->gpio_pmic_vcom_ctrl=0x%X \n",tps6518x->gpio_pmic_vcom_ctrl );
		ret = devm_gpio_request_one(&pdev->dev, CD4052_A0,
					GPIOF_OUT_INIT_LOW, "epdc-vcom");
		if (ret < 0)
			goto err;

	CD4052_A1= of_get_named_gpio(pmic_np,
							"CD4052_A1", 0);
			if (!gpio_is_valid(CD4052_A1)) {
				dev_err(&pdev->dev, "no epdc pmic vcom_ctrl pin available\n");
				goto err;
			}
		//	printk("\n*************tps6518x->gpio_pmic_vcom_ctrl=0x%X \n",tps6518x->gpio_pmic_vcom_ctrl );
			ret = devm_gpio_request_one(&pdev->dev, CD4052_A1,
						GPIOF_OUT_INIT_LOW, "epdc-vcom");
			if (ret < 0)
				goto err;
...
//	printk("\n*************tps6518x->gpio_pmic_vcom_ctrl=0x%X \n",tps6518x->gpio_pmic_vcom_ctrl );
	...
	printk("\n*****tps6518x_pmic_dt_parse_pdata********tps6518x->gpio_pmic_powerup=0x%X \n",tps6518x->gpio_pmic_powerup );	
...
//	printk("\n*************tps6518x->gpio_pmic_intr=0x%X \n",tps6518x->gpio_pmic_intr );		
...
//	printk("\n*************tps6518x->gpio_pmic_pwrgood=0x%X \n",tps6518x->gpio_pmic_pwrgood );			
...
void readgpio(void)
{
	...
	p_gpj0_base = ioremap(GPIO2BASE, 8);
	if (!p_gpj0_base)
		return;
	data=readl(p_gpj0_base);
//	printk("wym readgpio GPIO2 data:%x\n",data);
	data=readl(p_gpj0_base+4);
	iounmap(p_gpj0_base);
	p_gpj0_base = NULL;
//	printk("wym readgpio GPIO2 dir:%x\n",data);

	tps6518x_reg_read(REG_TPS65185_ENABLE, &data);
//	printk("wym readgpio REG_TPS65185_ENABLE :%x\n",data);
}
...
#if  1  //add by yan2anfei	
	if( 0 == tps6518x_reg_read(REG_TPS6518x_REVID,&revid_temp) )	{
		tps6518x->revID = revid_temp;
	//	printk("\n NNNNNNNNNNNNtps6518x->revID =0x%X\n ",tps6518x->revID);
	}
	else {
		dev_err(&pdev->dev, " tps6518x_reg_read failed revid_temp= %d\n",revid_temp);
	}
#endif
...
/*
	tps6518x_vcom_set_voltage(rdev[1],
		-5000000,
		tps6518x->vcom_uV,
		NULL);
		*/
...
static int __init tps6518x_setup(char *options)
{
	...
	while ((opt = strsep(&options, ",")) != NULL) {
		...
		if (!strncmp(opt, "vcom=", 5)) {
			...
			if (ret < 0)
				return ret;
			//printk("00000000  tps6518x_vcom: %d", tps6518x_vcom);
